(function() {
    ...
    function sendVisitorData() {
        ...
        // Envia apenas se houver pelo menos um Click ID
        if (Object.keys(clickIds).length > 0) {
            console.log("Dados a serem enviados:", clickIds);
            ...
            fetch(originUrl + '/wp-admin/admin-ajax.php', {
                ...
                body: new URLSearchParams(clickIds)
            })
            .then(response => response.json())
            .then(data => console.log('Resposta do servidor:', data))
            .catch(error => console.error('Erro ao enviar dados do visitante:', error));
        } else {
            console.log("Nenhum Click ID encontrado para enviar.");
        }
    }
